...
async function initNativeGoogleAuth() {
  try {
    ...
    if (import.meta.env.DEV)
      console.error('[login] native GoogleAuth.initialize failed:', err);
...
async function signInWithGoogleNative() {
  ...
  try {
    ...
    if (!accessToken)
      throw new Error(
        'Google access token missing from native sign-in response',
      );
    ...
    if (import.meta.env.DEV)
      console.error('[login] native sign-in error:', err);
    isLoading.value = false;
  }
}
...
async function _finaliseLogin(googleAccessToken: string) {
  try {
    ...
    if (import.meta.env.DEV)
      console.error('[login] _finaliseLogin error:', err);
...
function signInWithGoogle() {
  if (Capacitor.isNativePlatform()) {
    if (!sdkReady.value) {
      errorMsg.value =
        'Google SDK가 아직 초기화되지 않았습니다. 잠시 후 다시 시도해 주세요.';
      return;
    }
    ...
    if (!tokenClient) {
      errorMsg.value =
        'Google SDK가 아직 로드되지 않았습니다. 잠시 후 다시 시도해 주세요.';
      return;
    }
    isLoading.value = true;
    errorMsg.value = '';
...
<template>
  <div class="min-h-screen bg-surface flex flex-col px-6">
    ...
    <!-- ── Legal footer ── -->
    <p
      class="mb-10 text-xs2 text-text-disabled text-center leading-relaxed px-4"
    >
      계속 진행하면 Tikkle의
